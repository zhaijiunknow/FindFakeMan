namespace PhotoshopFile
{
    ...
    public class Layer
    {
        ...
        public Layer(BinaryReverseReader reader, PsdFile psdFile)
        {
            ...
            foreach (AdjustmentLayerInfo adjustmentLayerInfo in AdjustmentInfo)
            {
                if (adjustmentLayerInfo.Key == "TySh")
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning("Failed to read text layer data for layer '" + Name + "': " + ex.Message);
                    }
                }
        ...
        private void ReadTextLayer(BinaryReverseReader dataReader)
        {
            ...
            if (dataReader.Seek("/Justification "))
            {
            ...
            // read the font size
            if (dataReader.Seek("/FontSize "))
            {
                FontSize = dataReader.ReadFloat();
            }

            // read the font fill color
            if (dataReader.Seek("/FillColor") && dataReader.Seek("/Values [ "))
            {
            ...
            // read the font name
            if (dataReader.Seek("/FontSet ") && dataReader.Seek("/Name"))
            {
                dataReader.ReadBytes(4);
            ...
            // read the warp style
            if (dataReader.Seek("warpStyle") && dataReader.Seek("warpStyle"))
            {
                dataReader.ReadBytes(3);
